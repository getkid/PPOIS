...
TEST(TestGameForPC, Test10) {
	...
	game_for_PC.AddRequiredPC(s1, RAM, s2, free_space);
	...
	EXPECT_EQ(game_for_PC.GetRequiredPC().GetProcessor(), "ryzen 5 2600");
	EXPECT_EQ(game_for_PC.GetRequiredPC().GetFreeMemory(), 40);
	EXPECT_EQ(game_for_PC.GetRequiredPC().GetGraphicsCard(), "rtx 2050");
}
